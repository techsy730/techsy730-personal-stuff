 ...
 * 
 */
public final class MSDStringSorterWithCopyAvoidance
{
    ...
    private static final int _MAX_UPDATE_MAX_LENGTH_COMPUTE = (int)(ARRAYS_SORT_CHECK * 1.8);
...
*/
    ...
    private MSDStringSorterWithCopyAvoidance(Deque<SortState> stack, String[] arr, String[] wc, BitSet flippedTracker)
    {
        this.stack = stack;
        this.arr_local = arr;
        this.wc_local = wc;
        this.isArrAndWcFlipped = flippedTracker;
    }
    ...
    public static void sortPartially(int minCharsToSort, String[] arr, int fromIndex, int toIndex)
    {
        ...
        stack.addFirst(new SortState(fromIndex, toIndex, 0, minCharsToSort, false));
        //The thing that will hold our sort progress
        new MSDStringSorterWithCopyAvoidance(stack, arr, wc, new BitSet(toIndex - fromIndex)).doSort();
    }

    private final Deque<SortState> stack;
    private final String[] arr_local;
    private final String[] wc_local;
    private final BitSet isArrAndWcFlipped;
    ...
    private final void doSort()
    {
        ...
        //Now, for each region that has the final result written into the working copy, copy that back over to the real array.
        for(int i = isArrAndWcFlipped.nextSetBit(0), max = isArrAndWcFlipped.length(); i < max && i > 0;)
        {
            //Alright, find the end of this "block"
            int end = isArrAndWcFlipped.nextClearBit(i);
            //Now copy the wc back into the real array for this block
            System.arraycopy(wc_local, i, arr_local, i, end - i);
            //Now, find the next "block"
            i = isArrAndWcFlipped.nextSetBit(end);
        }
    ...
    private void processPart(SortState currentState)
    {
        ...
        boolean isFlipped = currentState.isFlipped;
        ...
        int[] buckets = doSortOnPart(start, end, index, maxIndex, needMaxIndexUpdate, isFlipped);
        if(buckets != INDEX_SORTED_NO_BUCKETS)
        {
            //If bucket sort was used, then we are now flipped
            isFlipped = !isFlipped;
        }
        isArrAndWcFlipped.set(start, end, isFlipped);
        ...
        final String[] arr = getArr(isFlipped);
                             ...
                             // be sorted anymore, as there is no data in the next index
        {
            ...
            else
            {
                ...
                if(buckets == INDEX_SORTED_NO_BUCKETS)
                    splitByCharGroups(start, end, index, maxIndex, curChar, currentState, needMaxIndexUpdate, isFlipped);
                else
                    splitByCharGroups(start, end, index, maxIndex, buckets, currentState, needMaxIndexUpdate, isFlipped);
            }
        }
    }
    
    private void splitByCharGroups(final int start,
        final int end, final int index, final int maxIndex, final char curChar, SortState currentState, final boolean updateMaxSize, final boolean isFlipped)
    {
            ...
            for(int i = newStart; i < end; ++i)
            {
                if(charAt(arr[i], index) != curChar2)
                {
                    ...
                    if(newSize > 1) //Don't even bother with a method call if our size is zero or one
                        doRecurse(lastNewIndex, i, newSize, index, maxIndex, curChar2, updateMaxSize, shouldMerge, maxMergedRangeSize, isFlipped);
                    lastNewIndex = i;
        ...
        else
        {
            ...
            for(int i = start + 1; i < end; ++i)
            {
                ...
                if(charAt(arr[i], index) != curChar2)
                {
                    ...
                    final int newSize = i - lastNewIndex;
                    if(newSize > 1) //Don't even bother with a method call if our size is zero or one
                        doRecurse(lastNewIndex, i, newSize, index, blockMaxSize, curChar2, false, shouldMerge, maxMergedRangeSize, isFlipped);
                    lastNewIndex = i;
                    curChar2 = charAt(arr[i], index);
        ...
        if(newSize > 1)
            doRecurseFinalIteration(lastNewIndex, end, newSize, index, maxIndex, curChar2, currentState, updateMaxSize, shouldMerge, maxMergedRangeSize, isFlipped);
    ...
    private void splitByCharGroups(final int start, final int end,
        final int index, final int maxIndex, final int[] buckets, SortState currentState, final boolean updateMaxSize, final boolean isFlipped)
    {
        @SuppressWarnings("hiding") //We are trying to shift from a instance member lookup to a local variable lookup for performance 
        final String[] arr = getArr(isFlipped);
        tempHolder = currentState;
        ...
        for(int i = 0; i < buckets.length - 1; ++i)
        {
            ...
            if(newSize > 1) //Don't even bother with a method call if our size is zero or one
                doRecurse(regionStart, regionEnd, newSize, index, maxIndex, charAt(arr[regionStart], index), updateMaxSize, shouldMerge, maxMergedRangeSize, isFlipped);
        }
        ...
        final int newSize = regionEnd - regionStart;
        if(newSize > 1)
            doRecurseFinalIteration(regionStart, regionEnd, newSize, index, maxIndex, charAt(arr[regionStart], index), currentState, updateMaxSize, shouldMerge, maxMergedRangeSize, isFlipped);
    ...
    private final void doRecurse(final int regionStart, final int regionEnd, final int newSize, final int index,
        ...
        final int maxMergedRangeSize, final boolean isFlipped)
    {
        @SuppressWarnings("hiding")
        final String[] arr = getArr(isFlipped);
        assert newSize > 1;
        ...
        switch(newSize)
        {
            ...
            default:
            {
                int maxIndexBlock = conditionalFindMaxIndexBlock(regionStart, regionEnd, newSize, maxIndex, updateMaxSize, maxMergedRangeSize, isFlipped);
                trackMergeAndAdd(regionStart, regionEnd, newSize, index, maxIndexBlock, shouldMerge, maxMergedRangeSize, isFlipped);
            }
        }
    }
    ...
    private final void doRecurseFinalIteration(int regionStart, final int regionEnd, final int newSize,
        ...
        final boolean shouldMerge, final int maxMergedRangeSize, final boolean isFlipped)
    {
        @SuppressWarnings("hiding")
        final String[] arr = getArr(isFlipped);
        assert newSize > 1;
        ...
        switch(newSize)
        {
            ...
            {
                int maxIndexBlock = conditionalFindMaxIndexBlock(regionStart, regionEnd, newSize, maxIndex, updateMaxSize, maxMergedRangeSize, isFlipped);
                trackMergeAndAdd(regionStart, regionEnd, newSize, index, maxIndexBlock, shouldMerge, maxMergedRangeSize, isFlipped, currentState);
            }
        }
    }
    ...
    private final void
        trackMergeAndAdd(final int regionStart, final int regionEnd, final int newSize,
            ...
            final int maxMergedRangeSize, final boolean isFlipped)
    {
        trackMergeAndAdd(regionStart, regionEnd, newSize, index, maxIndexBlock, shouldMerge, maxMergedRangeSize, isFlipped, new SortState());
    }
    
    private final void
        trackMergeAndAdd(final int regionStart, final int regionEnd, final int newSize,
            final int index, final int maxIndexBlock, final boolean shouldMerge,
            final int maxMergedRangeSize, final boolean isFlipped, SortState toUseState)
    {
        ...
        if(shouldMerge)
        {
            if(newSize < MAX_INSERTION_SORT && newSize >= MIN_BLOCK_LEN_TO_MERGE)
            {
                ...
                SortState newState =
                    toUseState == tempHolder ?
                        new SortState(regionStart, regionEnd, index, maxIndexBlock, isFlipped) :
                        toUseState.setState(regionStart, regionEnd, index, maxIndexBlock, isFlipped);
            ...
            {
                flushMergedIfNeeded();
                stack.addFirst(toUseState.setState(regionStart, regionEnd, index + 1, maxIndexBlock, isFlipped));
            }
        }
        else
        {
    ...
    private final String[] getArr(boolean isFlipped)
    {
        return isFlipped ? this.wc_local : this.arr_local;
    }
    
    private final String[] getWc(boolean isFlipped)
    {
        return isFlipped ? this.arr_local : this.wc_local;
    }
    ...
    private int[] doSortOnPart(final int fromIndex, final int toIndex,
        final int charIndex, final int maxIndex, final boolean tryTrackMaxIndex, final boolean isFlipped)
    {
        ...
        return bucketSort(fromIndex, toIndex, charIndex, tryTrackMaxIndex, isFlipped);
    }
        ...
        final int charIndex, final int maxIndex)
    {
        ...
        //Is there a faster way to do this than an O(n*m) algorithm?
    ...
    private int[] bucketSort(final int fromIndex, final int toIndex,
        final int charIndex, final boolean trackMaxLen, final boolean isFlipped)
    {
        //Note The SortState is not given for from, to, etc, but rather for updating the max seen character 
        final int len = toIndex - fromIndex;
        @SuppressWarnings("hiding") //We are trying to shift from a instance member lookup to a local variable lookup for performance 
        final String[] arr = getArr(isFlipped);
        @SuppressWarnings("hiding") //We are trying to shift from a instance member lookup to a local variable lookup for performance
        final String[] wc = getWc(isFlipped);
    ...
    private static final class SortState implements Cloneable
    {
        ...
        SortState(final int start, final int end, final int index, final int maxIndex, final boolean isFlipped)
        {
            this.start = start;
            this.end = end;
            this.index = index;
        ...
        SortState(final SortState otherState)
        {
            ...
            this.isFlipped = otherState.isFlipped;
        }
        
        public SortState setState(final int start, final int end, final int index, final int maxIndex, final boolean isFlipped)
        {
            this.start = start;
            this.end = end;
            this.index = index;
        ...
        {
            return "[" + start + ", " + end + ") @ " + index + " (max: " + maxIndex + ')' + (isFlipped ? "[flipped arrays]" : "");
        }
        ...
        public SortState mergeWith(final SortState other)
        {
            ...
            if(isFlipped != other.isFlipped) return null;
        ...
        {
            return new SortState(start, end, index, maxIndex, isFlipped);
        }
    ...
    private static final boolean ONLY_ONE_RUN = true;
